...
if(@ARGV > 13) 
{
	...
	print "\n Note: rho_p used only to calculate prevalence of disorder1&2 co-occurrence, so don't set above 0";
	exit(1);
}
...
my $prev_bivariate = `python ./stupid_bivariate_prevalence_program.py $thres[0] $thres[1] $rho_p` + 0;
my $thresh_bivariate = inv_cdf(1.0 - $prev_bivariate);
print FILE_SS "\n Bivariate 'phenotype' 1&2 has a prev = $prev_bivariate and thres = $thresh_bivariate \n";
# prev_both = tensor(pd_both_generator.cdf(disease_z_scores))
# print FILE_SS "\n Disorder $i has a prev = $prev[$i] and thres = $thres[$i] \n";

print FILE_SS "\n Fraction of genes that affect 1: $ARGV[5], 2: $ARGV[6], 1&2: $ARGV[7]";
