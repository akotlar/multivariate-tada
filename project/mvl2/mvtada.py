...
def modelPoisson(
    ...
    #    lmbda = numpyro.sample("lambda",
    #        Gamma(5.*np.ones(4),5.*np.ones(4)))
    lmbda = numpyro.sample("lambda",Gamma([[5.0,5.0,5.0,5.0]],[[5.0,5.0,5.0,5.0]]))
    ...
    with numpyro.plate("data", data.shape[0]):
        ...
        print(lmbda.shape)
        print(data.shape)
        return numpyro.sample("obs", ProductPoisson(lmbda_z), obs=data)
