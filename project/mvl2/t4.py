...
def model(K,dim,data=None):
    ...
    with numpyro.plate('data', len(data)):
        z= numpyro.sample('z', dist.Categorical(cluster_proba),infer={"enumerate": "parallel"}) 
        print(locs[z].shape)
        print(sigma[z].shape)
        cov = numpyro.deterministic("cov",sigma*jnp.eye(dim))
        print(cov[z].shape)
        numpyro.sample('obs', dist.MultivariateNormal(locs[z], cov[z]), obs=data)
