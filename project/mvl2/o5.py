...
s for it.
    ...
    with numpyro.plate("lambda_plate", k_hypotheses):
        lmbda = numpyro.sample("lambda", Gamma(5.0*np.ones(4), 5.0*np.ones(4)))

    print('A',lmbda.shape)

    pz = numpyro.deterministic("pz", mix_weights(beta))

    with numpyro.plate("individuals", data.shape[0]):
        ...
        print(lmbda.shape)
        print(lmbda[:,z].shape)
        print(jnp.transpose(lmbda[:,z]).shape)
        print('>>>>>>>>>>>>>>>')
        print('>>>>>>>>>>>>>>>')
...
class MultiVariateZITada(object):
    def __init__(self, max_K=4, training_options={}):
        self.max_K = int(max_K)
        self.training_options = self._fill_training_options(training_options)
...
def modelZIPoisson(
    ...
    with numpyro.plate("lambda_plate", k_hypotheses):
        ...
        gates = numpyro.sample("gates", Beta(1.0, 1.0))
    ...
    with numpyro.plate("individuals", data.shape[1]):
        with numpyro.plate("data", data.shape[0]):
            ...
            return numpyro.sample(
                "obs", ZeroInflatedPoisson(gates_z, lmbda_z), obs=data
            )
