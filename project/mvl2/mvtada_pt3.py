...
class MVTadaPT(object):
    ...
    def fit(self, data, progress_bar=True,Lamb_init=None,pi_init=None):
        ...
        for i in myrange(td["n_iterations"]):
            ...
            loglikelihood_components = torch.transpose(loglikelihood_stack,0,1) + torch.log(pi_)
            loglikelihood_marg = torch.logsumexp(loglikelihood_components,dim=1)
            #print('>>>>>>>>>>>>>>>>>>>>>>')
            #print('>>>>>>>>>>>>>>>>>>>>>>')
            #print('>>>>>>>>>>>>>>>>>>>>>>')
            #print(loglikelihood_marg.shape)
            #print('>>>>>>>>>>>>>>>>>>>>>>')
            #print('>>>>>>>>>>>>>>>>>>>>>>')
