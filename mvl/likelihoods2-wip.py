...
def effectLLDMD(nHypotheses, pDs, altCountsFlat):
    ...
    print('altCountsShaped', altCountsShaped.shape)
...
def effectLikelihood(nHypotheses, pDs, altCountsFlat, nCases: Tensor, nCtrls: Tensor):
    from mvl.GeneralizedDirichlet import dgdirmn
    ...
    altCountsShaped = altCountsFlat.expand(nHypotheses, nGenes, nConditions).transpose(0, 1)
    print("altCountsShaped", altCountsShaped[:, 1:, :].shape)
    # altCountsShapedRepeat = torch.repeat_interleave(altCountsShaped, torch.tensor([1, 1, 2]), 1)  
    nShaped = n.expand(nHypotheses, nGenes).T
    nShapedRepeat = n.expand(nHypothesesNonNull + 1, nGenes).T
    # print("nShaped", nShaped)
    samplePDs = nCases / (nCases.sum() + nCtrls)
    pdsAll = tensor([1 - samplePDs.sum(), *samplePDs])
    # print("pdsAll", pdsAll)
    # pdsAllPop = tensor([1 - pDs.sum(), *pDs])
    # print("pdsAllPop", pdsAllPop)
    # pdsAll = pdsAllPop
    print("pdsAll", pdsAll)
    pdsAllShaped = pdsAll.expand(nHypotheses, nConditions)
    # pdsAllShapedRepeat = pdsAll.expand(nHypotheses, nConditions)
    ...
    # H1&2
    # H1&2&3
    def likelihoodFn2(alpha0, alpha1):
        ...
        # print("pdsAllShaped[0:2, :]", pdsAllShaped[0:2, :].shape)
        # print("nShaped[:, 2]", nShaped[:, 2].shape)
        # print("altCountsShaped[:, 0:2]", altCountsShaped[:, 0:2].shape)
        # print("nShaped", nShaped.shape)
        # print("nShaped[:, 0:2]", nShaped[:, 0:2].min())
        # print("altCountsShaped[:, 0:2].min", altCountsShaped[:, 0:2].min())
        # try to stay in the log space; avoid numeric underflow, until bayes factor calc
        return torch.exp(DirichletMultinomial(total_count=10, concentration=concentrations).log_prob(altCountsShaped[:, 0:2]))

    def likelihoodFn22(alpha0, alpha2):
        concentrations = pdsAllShaped[0:2] * tensor([
            [alpha0, alpha0, alpha0, alpha0],
            [alpha0, alpha0, alpha2, alpha2],  # H1
        ]).expand(nGenes, 2, 4)
        # print("concentrations", concentrations.shape)
        # print("pdsAllShaped[0:2, :]", pdsAllShaped[0:2, :].shape)
        # print("nShaped[:, 2]", nShaped[:, 2].shape)
    ...
    def likelihoodFn221(alpha0, alpha2):
        concentrations = pdsAllShaped[0:2] * tensor([
    ...
    def likelihoodFn3(alpha1, alpha2, alphaBoth):
        concentrations = pdsAllShaped[0:2] * tensor([
            [alpha0, alpha1, alpha0, alpha1],
            [alpha0, alpha1, alpha2, alphaBoth],  # H1
        ]).expand(nGenes, 2, 4)
        # print("concentrations", concentrations.shape)
        # print("pdsAllShaped[0:2, :]", pdsAllShaped[0:2, :].shape)
        # print("nShaped[:, 2]", nShaped[:, 2].shape)
    ...
    def likelihoodGDM(alphas, betas):
        alphas = pdsAllShaped * tensor([
            [alphas[0], alphas[1], alphas[0], alphas[1]],  # H1
            [alphas[0], alphas[0], alphas[2], alphas[2]],  # H1
            [alphas[0], alphas[1], alphas[2], alphas[3]],  # HBoth
        ]).expand(nGenes, nHypothesesNonNull, nConditions)
        # print("alphas", alphas.shape)
        betas = pdsAllShaped * tensor([
            [betas[0], betas[1], betas[0], betas[1]],  # H1
            [betas[0], betas[0], betas[2], betas[2]],  # H1
            [betas[0], betas[1], betas[2], betas[3]],  # HBoth
        ]).expand(nGenes, nHypothesesNonNull, nConditions)
        # print("betas", betas.shape)
        # print("altCountsShaped", altCountsShaped.T.shape)
        return torch.exp(dgdirmn(altCountsShaped, alphas, betas))
    ...
    def likelihoodFn(alpha0, alpha1, alpha2, alphaBoth):
        ...
        # total sums to 1
        # and P(D0|V)P(V) + P(D1|V)P(V)
        # print("new", pdsAllShaped.shape, nHypotheses)
        concentrations = pdsAllShaped * tensor([
            #ctrls, cases1, cases2, casesBoth
            # keep reuse; to reflec the fact that we're not truly multinomial
            # these are really independent binomial

            # #
            #P(D1only|V) = (alpha0 + xCase1) * pi0 + alpha1 * pi1 + alpha0 * pi2 + alpha1 * pi3
            #P(D1|V) = P(D1only|V,architecture) =       P(architecture) = 1
            #P(D1|V) = pi0 * N_Case1 / N_all                ##########(alpha0 + N_case1) / (alpha_all + N_all)) * pi_0 + (alpha1 + X_case1 + X_caseBoth)  * pi1/(pi1+piBoth) + (alpha_1 + x_case1 + alpha_both + x_both) * piBoth/(pi1+piBoth)
            # [alpha0, alpha0, alpha0, alpha0],
                                                            #### # P(D1|V, component0) = (alpha0 + Xcase1) / (alpha0*4 + X_all_in_gene) ~ x_case1/denom
            # P(D1|V, component1) = pi1 * (2alpha1 + x_case1 + x_caseBoth) / (denom)
            # P(D1|V, component2) = pi2 * (alpha02 + x_case1 + alpha2 + x_caseBoth) / (all_alphas + x_all)
            # P(D1|V, componentBoth) = piBoth * (alpha1 + x_case1 + alphaBoth + xCaseBoth) / (...)
            [alpha01, alpha1, alpha01, alpha1],  # H1 alpha1/(alpha0 + alpha1 + alpha2 + alpha1)
            [alpha02, alpha02, alpha2, alpha2],  # H2
            # alpha_sum = a0 + a1 + aBoth + a2 + aBoth + a1 + a2 + aBoth
            # E(P(V|D)P(V)) = alpha1 + alphaBoth / alpha_sum
            [alpha03, alpha1, alpha2, alphaBoth]  # H1&2&3 P(V|D1) alpha1/(alpha0 + alpha1 + alpha2 + alphaBoth)
        ]).expand(nGenes, nHypotheses, nConditions)
        ...
        # try to stay in the log space; avoid numeric underflow, until bayes factor calc
        return torch.exp(DirichletMultinomial(total_count=nShaped[1:, ], concentration=concentrations).log_prob(altCountsShaped[:, 1:, :]))
    ...
    def likelihoodFnOld(alpha0, alpha1, alpha2, alphaBoth):
        ...
        concentrations = pdsAllShaped * tensor([
            #ctrls, cases1, cases2, casesBoth
            [alpha0, alpha0, alpha0, alpha0],  # H0
            [alpha0, alpha1, alpha0, alpha1],  # H1
            [alpha0, alpha0, alpha2, alpha2],  # H2
            # alpha_sum = a0 + a1 + aBoth + a2 + aBoth + a1 + a2 + aBoth
            # E(P(V|D)P(V)) = alpha1 + alphaBoth / alpha_sum
    ...
    return likelihoodFn, nullLikelihood(pdsAll, altCountsFlat), likelihoodFnSimpleNoLatent, likelihoodFnBoth, likelihoodFn2, likelihoodFnAlphaEverywhere, likelihoodFnOld, likelihoodGDM, likelihoodFn22, likelihoodFn221, 
...
def likelihoodBivariateFast(altCountsFlat, pDs, nCases: Tensor, nCtrls: Tensor, trajectoryPis, trajectoryAlphas, trajectoryLLs):
    ...
    nHypotheses = 4
    nGenes = altCountsFlat.shape[0]
    print("pDs in likelihoodBivariateFast", pDs)
    likelihoodFn, allNull2, likelihoodFnNoLatent, likelihoodBothModels, likelihoodFn2, likelihoodFnA0, likelihoodFnOld, likelihoodGDM, likelihood22, likelihood221 = effectLikelihood(
    ...
    def llGDM(params):
        ...
        trajectoryAlphas.append([alphas, betas])

        hs = tensor([pis]) * likelihoodGDM(alphas, betas)
    ...
    #     # if alpha1 > 500 or alpha2 > 500 or alphaBoth > 500:
    #     #     print("returning inf due to alphas > 500", params)
    #     #     return float("inf")
    ...
    #     hs = tensor([[pi0, pi1, pi2, piBoth]]) * likelihoodFn(alpha0, alpha1, alpha2, alphaBoth)

    #     ll = -torch.log(hs.sum(1)).sum()
    #     # print("ll", ll)
    #     trajectoryLLs.append(ll)
    #     return ll
    ...
    def jointLikelihoodOld(params):
        ...
        trajectoryPis.append([pi0, pi1, pi2, piBoth])
        trajectoryAlphas.append([alpha0, alpha1, alpha2, alphaBoth])

        hs = tensor([[pi0, pi1, pi2, piBoth]]) * likelihoodFnOld(alpha0, alpha1, alpha2, alphaBoth)
    ...
    def jointLikelihoodSimple(params):
        ...
        trajectoryPis.append([pi1])
        trajectoryAlphas.append([alpha0, alpha1])

        hs = tensor([[pi0, pi1]]) * \
            likelihoodFn2(alpha0, alpha1)
        # print("hs", hs)
        ll = -torch.log(hs.sum(1)).sum()
        # print("ll",ll)
        if ll == float("inf"):
    ...
    def jointLikelihood21(params):
        pi1, pi2, alpha1, alpha2 = params
    ...
    def jointLikelihood3(params):
        pi2, alpha0, alpha2 = params
    ...
    return jointLikelihood, jointLikelihoodSimple, jointLikelihoodBoth, jointLikelihood2, jointLikelihoodDirichlet, jointLikelihoodSimpleDirichlet, jointLikelihoodA0, jointLikelihoodOld, llGDM
...
def fitFnBivariate(altCountsByGene, pDs, nCases: Tensor, nCtrls: Tensor, nEpochs=1, minLLThresholdCount=100, K=4, debug=False, method="nelder-mead", old=False, gdm=False):
    trajectoryPis = []
    trajectoryAlphas = []
    trajectoryLLs = []
    trajectoryPisSimple = []
    trajectoryAlphasSimple = []
    trajectoryLLsSimple = []
    costFn, costFnSimple, costFnBoth, _, _, _, costFnA0, costFnOld, costFnGDM = likelihoodBivariateFast(
...
def fitFnBivariateStacked(altCountsByGene, pDs, nCases, nCtrls, K=4, debug=False, **kwargs):
    trajectoryPis = []
    trajectoryAlphas = []
    trajectoryLLs = []
    trajectoryPisSimple = []
    ...
    for y in range(10):
        pi0 = pi0Dist.sample()
        pis = Uniform(1/nGenes, 1-pi0).sample([1])
        pis = pis/(pis.sum() + pi0)
        fnArgs = [*pis.numpy(), *alphasDist.sample([2, ]).numpy()]
        print("fnArgs, ", fnArgs)
    ...
    print("bestParamsSimple", bestParamsSimple)
    # with Pool(2) as p:
    # results = []
    start = time.time()
